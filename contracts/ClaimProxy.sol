...
import "./ERC735.sol";
        ...
        // pendingClaims[claimRequestId].topic = _topic;
        // pendingClaims[claimRequestId].scheme = _scheme;
        // pendingClaims[claimRequestId].issuer = _issuer;
        // pendingClaims[claimRequestId].signature = _signature;
        // pendingClaims[claimRequestId].data = _data;
        // pendingClaims[claimRequestId].uri = _uri;
...
contract ClaimProxy is ERC735 {
    ...
    IdentityProxy public identityAddress; // = new IdentityProxy(1);

    constructor(address identity) {
        identityAddress = IdentityProxy(identity);
    }
    ...
    function addClaim(uint256 _topic, uint256 _scheme, address _issuer, bytes _signature, bytes _data, string _uri) public returns (uint256 claimRequestId) {
      ...
      if (msg.sender != address(identityAddress)) {
          require(identityAddress.keyHasPurpose(keccak256(msg.sender), 1));
      }
